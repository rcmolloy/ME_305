...
;=============================================================================
;
;    Subroutine TASK_4            ; pattern_4

TASK_4: ldaa   t4state            ; get current t1state and branch accordingly
        beq    t4state0
        deca
        beq    t4state1
        deca
...
TASK_5: ldaa   t5state            ; get current t2state and branch accordingly
        beq    t5state0
        deca
        beq    t5state1
		deca
...
t5state1:                         ; (re)initialize COUNT_1
        ...
        ldx    COUNT_2
        dex                       ; decrement COUNT_1
        stx    COUNT_2            ; store decremented COUNT_1
		clr    DONE_2
        movb   #$02, t5state      ; set next state
        rts

t5state2:                         ; count down COUNT_1
        ldx    COUNT_2
        beq    setdone_2          ; test to see if COUNT_1 is already zero
        dex                       ; decrement COUNT_1
        stx    COUNT_2            ; store decremented COUNT_1
