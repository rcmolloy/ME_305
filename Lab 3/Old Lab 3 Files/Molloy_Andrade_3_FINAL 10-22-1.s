...
keyFlag::         .blkb 1           ; Key Flag for Digit, F1, F2, Enter, or BSpace
...
mmstate2:
         tst      keyFlag            ; Test keyFlag variable to see if 0 or 1         
	   beq      mmHUBLOOP          ; If no key pressed (keyFlag = 0), loop back through Mastermind Hub
         movb     #$00, keyFlag      ; Set keyFlag to 0 (False)
         ldab     keyBuffer          ; Load the ASCII value of keyBuffer into B
         cmpb     #$F1               ; Compare accumulator B to F1
         beq      initF1             ; If F1 is B, then branch to the F1_Subroutine
...
initF1:
       movb   #$03, mmstate      ; Set next state to initF1
       rts        

mmstate3:                         ; Initialize F1
       tst    F2FLAG              ; Test F2FLAG
	 bne    BOTTOM1             ; Branch to BOTTOM1 if F2FLAG is TRUE
       movb   #$00, DIGIT_COUNT   ; Clear DIGIT_COUNT
	 movb   #$00, DISP_COUNT    ; Clear DISP_COUNT
       jsr    CLEAR_BUFFER        ; Jump to CLEAR_BUFFER, subroutine
	 movb   #$01, F1FLAG        ; Set F1Flag to TRUE
	 movb   #$00, F2FLAG        ; Set F2Flag to FALSE
       movb   #$01, DF1PROMPT     ; Set DF1PROMPT to TRUE
       movb   #$02, t1state       ; Set next state to: M^2 HUB
	   rts
BOTTOM1:
       movb   #$02, t1state       ; Set next state to: M^2 HUB
